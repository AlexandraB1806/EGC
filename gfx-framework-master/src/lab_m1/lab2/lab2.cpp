...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            // 1. pozitie
            // 2. culoare
            VertexFormat(glm::vec3(0, 0,  1), glm::vec3(0.2, 0.8, 0.6)), // 0
            VertexFormat(glm::vec3(-1, 0,  1), glm::vec3(0.5, 1, 0.7)), // 1
            VertexFormat(glm::vec3(0, -1,  1), glm::vec3(0.3, 0.1, 0.8)), // 2
            VertexFormat(glm::vec3(-1, -1,  1), glm::vec3(1, 2.1, 0.2)), // 3
            VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0.2, 0.1, 0.6)), // 4
            VertexFormat(glm::vec3(-1, 0,  0), glm::vec3(1.2, 1.8, 0.6)), // 5
            VertexFormat(glm::vec3(0, -1,  0), glm::vec3(0.2, 0.9, 0.2)), // 6 
            VertexFormat(glm::vec3(-1, -1,  0), glm::vec3(0.4, 0.7, 0.6)), // 7
        };
    ...
    // Bonus: circle
    {
        ...
        {
            //arg = TWO_PI / NUM_TRIANGLES * i;
            vertices.emplace_back(glm::vec3(cos(TWO_PI / NUM_TRIANGLES * i), sin(TWO_PI / NUM_TRIANGLES * i), 0), glm::vec3(1, 0, 0));
            indices.push_back(i);
            indices.push_back(i);
        }
...
{
    // container (incapsuleaza VBO si IBO; pot fi f multe buffere VBO)
    unsigned int VAO = 0;
    // TODO(student): Create the VAO and bind it
    glGenVertexArrays(1, &VAO);
    // bind:
    glBindVertexArray(VAO); // de acum putem declara VBO si IBO
    ...
    // data upload from CPU to GPU
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices[0]) * vertices.size(), &vertices[0], GL_STATIC_DRAW);

    // buffer pt indicii vf
    unsigned int IBO = 0;
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    // cube generated with InitFromData
    ...
    // the same cube generated with VAO, VBO, IBO
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(1.5f, 0.5f, 0.f));
    ...
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(3, 3, 3));

    // TODO(student): Draw the circle
    RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(0, 2, 3));
