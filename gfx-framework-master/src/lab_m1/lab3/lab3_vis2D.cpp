...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_Z)) {
        // bring to center the left-down corner of the square
        logicSpace.x += logicSpace.width / 2;
        logicSpace.y += logicSpace.height / 2;

        // the dimension of the logic space gets smaller
        logicSpace.width -= deltaTime * 2;
        logicSpace.height -= deltaTime * 2;

        // bring back the corner to initial position
        logicSpace.x -= logicSpace.width / 2;
        logicSpace.y -= logicSpace.height / 2;
    }
    ...
    if (window->KeyHold(GLFW_KEY_X)) {
        ...
        // the dimension of the logic space gets bigger
        logicSpace.width += deltaTime * 2;
        logicSpace.height += deltaTime * 2;
