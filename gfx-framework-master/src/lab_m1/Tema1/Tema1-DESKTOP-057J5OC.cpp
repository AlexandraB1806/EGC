    ...
    if ((score < 100 && drawnScore < 240) || numLives > 0) {
        ...
        if ((currTime > 5 && duckShot == false) || (duckShot == true && ok == 1)) {
            ...
            if (translateY >= resolution.y) {
                ...
                // Resetare pentru urmatoarele rate (ele nu evadeaza, dar nici
                // nu vor fi impuscate neaparat)
                duckEscape = true;
                duckShot = false;
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_LEFT)) {
        ...
        if (duckEscape == false) {
            ...
            if (mouseX >= cornerHitBoxLeftDown.x && mouseX <= cornerHitBoxRightUp.x &&
                ...
                // Scorul nu se mai actualizeaza cand rata evadeaza / este impuscata,
                // chiar daca o nimeresc
                if (angle != RADIANS(270) && angle != RADIANS(90)) {
                    score += 5;
                    drawnScore += 12;
                }
